...
export function updateFragments(
	...
	elements.fragments.forEach((frag) => {
		...
		const audioOrbitBoost = audio.current * 0.12 + audio.surge * 0.12;
		const orbitSpeed =
		...
		const dynamicRadius = frag.orbitRadius + audio.current * 0.03 + audio.surge * 0.05;
		const bobAmount = 0.08 + intensity * 0.2 + audio.surge * 0.02;
		...
		const tumbleSpeed = 0.1 + intensity * 0.5 + audio.current * 0.1 + audio.surge * 0.12;
		frag.mesh.rotation.x += dt * tumbleSpeed;
		frag.mesh.rotation.y += dt * tumbleSpeed * 1.5;

		frag.material.opacity = clamp01(0.45 + intensity * 0.4 + audio.current * 0.08 + audio.surge * 0.08);
	});
}

...
export function updateRings(elements: SceneElements, state: RigState, dt: number, intensity: number): void {
	...
	elements.rings.forEach((ring, i) => {
		...
		const audioSpin = audio.current * 0.35 + audio.surge * (0.55 + i * 0.12);
		const ringSpeed =
			ring.speed * (0.4 + ringIntensity * 1.5 + audioSpin) + intensityState.spinBoost * (1 + i * 0.2);
		ring.mesh.rotateOnAxis(ring.axis, dt * ringSpeed);

		const wobbleSpeed = 3 + i * 0.5;
		ring.wobblePhase += dt * wobbleSpeed;
		const wobbleAmount = audio.current * 0.012 + audio.surge * 0.018;
		...
		if (intensity > 0.1) {
			...
			ring.material.opacity = clamp01(baseOpacity + wave + audio.current * 0.08 + audio.surge * 0.08);
		} else {
			ring.material.opacity = clamp01(baseOpacity + audio.current * 0.08 + audio.surge * 0.08);
		}
	});
}
